...
$(document).ready(function () {
    ...
    $('#select-lojas').select2({
        placeholder: "Escolha as lojas participantes",
        minimumInputLength: 2,
        theme: "bootstrap-5",
        ajax: {
            url: '/bandeira/lojas/list',
            dataType: 'json'
        },
        language: getSelect2LangBR(),
        dropdownParent: $("#main-content .card-body form"),
    });
});
...
function searchProduct(){
    ...
    if(value.length > 2){
        $.ajax({
            type: 'get',
            url: '/bandeira/pesquisa/produtos',
            data: {'ean': value},
            beforeSend: () => {
...
$("#formAddCampanha").on("submit", (e) => {
    ...
    $.ajax({
        ...
        success: (result) => {
            ...
            $("#eanQtd").val("");
            $("#eanValue").val("");
            $('#select-lojas').val(null).trigger('change');
            searchProduct();
            reload_products_list();
